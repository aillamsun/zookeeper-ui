...
public class ZkConnection {
    ...
    private Logger LogUtils = LoggerFactory.getLogger(ZkConnection.class);
        ...
        if (null == this._watcher) {
            this._watcher = new Watcher() {
                ...
                public void process(WatchedEvent event) {
                    LogUtils.info("我是一个空闲的 Watcher ，不要监听任何事件的 Watcher ！");
                }
            };
        } else {
            // 加锁
            ...
            try {
                ...
                LogUtils.info("创建一个 ZooKeeper instance connect to 【"
                        + _servers + "】.");
                try {
                    _zk = new ZooKeeper(_servers, _sessionTimeOut, _watcher);
                } catch (IOException e) {
                    LogUtils.error("Unable Zookeeper Instance connect to 【 "
                            + _servers + " 】", e);
                }
            } finally {
    ...
    public void close() {
        ...
        try {
            if (_zk != null) {
                LogUtils.info("Closing ZooKeeper connected to 【 " + _servers
                        + "】");
                _zk.close();
                _zk = null;
            }
        } catch (InterruptedException e) {
            LogUtils.error("Closing Zookeeper connected fail !", e);
        } finally {
            _zookeeperLock.unlock();
        }
    }
